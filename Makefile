...
aws-lambda-custom: check-zip wrapper
	...
	@echo -e "Install upstream dependencies without deps to include in layer"
	@/opt/python/cp38-cp38/bin/pip3.8 install -t ${target_dir}/nodeps -r lambda/requirements-nodeps.txt --no-deps
	@echo "Contents of /code/contrib-custom:"
	@ls -al /code/contrib-custom/
	@echo -e "Install botocore instrumentor from local to include in layer"
	@/opt/python/cp38-cp38/bin/pip3.8 install -t ${target_dir}/nodeps -Ie /code/contrib-custom/instrumentation/opentelemetry-instrumentation-botocore
	@echo -e "Directly copying botocore src files"
	@mkdir ${target_dir}/nodeps/opentelemetry/instrumentation/botocore
	@cp -r /code/contrib-custom/instrumentation/opentelemetry-instrumentation-botocore/src/opentelemetry/instrumentation/botocore/* ${target_dir}/nodeps/opentelemetry/instrumentation/botocore
	@echo -e "Install solarwinds_apm to be packed up in zip archive to target directory."
	@/opt/python/cp38-cp38/bin/pip3.8 install . -t ${target_dir}/nodeps --no-deps
	@echo -e "Removing non-lambda C-extension library files generated by pip install under target directory."
	@rm -rf ${target_dir}/nodeps/solarwinds_apm/extension/*.so*
