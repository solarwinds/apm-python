...
aws-lambda-custom: check-zip wrapper
	...
	@/opt/python/cp38-cp38/bin/pip3.8 install -t ${target_dir}/nodeps -Ie ../contrib-custom/opentelemetry-python-contrib/instrumentation/opentelemetry-instrumentation-aws-lambda
	@echo -e "Directly copying aws-lambda src files"
	@mkdir ${target_dir}/nodeps/opentelemetry/instrumentation/aws_lambda
	@cp -r ../contrib-custom/opentelemetry-python-contrib/instrumentation/opentelemetry-instrumentation-aws-lambda/src/opentelemetry/instrumentation/aws_lambda/* ${target_dir}/nodeps/opentelemetry/instrumentation/aws_lambda
	@echo -e "Install solarwinds_apm to be packed up in zip archive to target directory."
	@/opt/python/cp38-cp38/bin/pip3.8 install . -t ${target_dir}/nodeps --no-deps
	@echo -e "Removing non-lambda C-extension library files generated by pip install under target directory."
	@rm -rf ${target_dir}/nodeps/solarwinds_apm/extension/*.so*
