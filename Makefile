...
build_dir := "./solarwinds-apm-lambda"
build-SWOTelLayer: check-zip wrapper
	rm -rf ${build_dir}
	@echo -e "Creating target directory ${build_dir} for AWS Lambda layer artifacts."
	mkdir -p ${build_dir}/python
	@echo -e "Install upstream dependencies to include in layer"
	@/opt/python/cp38-cp38/bin/pip3.8 install -t ${build_dir}/python -r lambda/requirements.txt
	@echo -e "Install upstream dependencies without deps to include in layer"
	@/opt/python/cp38-cp38/bin/pip3.8 install -t ${build_dir}/nodeps -r lambda/requirements-nodeps.txt --no-deps
	@echo -e "Install solarwinds_apm to be packed up in zip archive to target directory."
	@/opt/python/cp38-cp38/bin/pip3.8 install . -t ${build_dir}/nodeps --no-deps
	@echo -e "Removing non-lambda C-extension library files generated by pip install under target directory."
	@rm ${build_dir}/nodeps/solarwinds_apm/extension/*.so*
	@echo -e "Building AWS Lambda version of C-extensions for all supported Python versions in target directory."
	@set -e; for PYBIN in cp37-cp37m cp38-cp38; do /opt/python/$${PYBIN}/bin/python setup.py build_ext_lambda -b ${build_dir}/nodeps; done
	@echo -e "Copying AWS Lambda specific Oboe library liboboe-1.0-lambda-${platform}.so into target directory."
	@cp solarwinds_apm/extension/liboboe-1.0-lambda-${platform}.so ${build_dir}/nodeps/solarwinds_apm/extension/liboboe.so
	@echo -e "Moving no-deps dependencies, needed for full opentelemetry/instrumentation path"
	@cp -r ${build_dir}/nodeps/* ${build_dir}/python && rm -rf ${build_dir}/nodeps
	@echo -e "Copying OpenTelemetry lambda wrapper and entry script into target directory."
	@cp lambda/otel_wrapper.py ${build_dir}/python/otel_wrapper.py
	@cp lambda/otel-instrument ${build_dir}/otel-instrument
	@chmod 755 ${build_dir}/otel-instrument
