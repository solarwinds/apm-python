...
install-lambda-modules-custom:
	...
	@echo -e "Install logging instrumentor from local to include in layer"
	@/opt/python/cp38-cp38/bin/pip3.8 install -t ${target_dir}/nodeps -Ie contrib-custom/instrumentation/opentelemetry-instrumentation-logging
	@echo -e "Directly copying logging src files"
	@mkdir ${target_dir}/nodeps/opentelemetry/instrumentation/logging
	@cp -r contrib-custom/instrumentation/opentelemetry-instrumentation-logging/src/opentelemetry/instrumentation/logging/* ${target_dir}/nodeps/opentelemetry/instrumentation/logging
	@echo -e "Install solarwinds_apm to be packed up in zip archive to target directory."
	@/opt/python/cp38-cp38/bin/pip3.8 install . -t ${target_dir}/nodeps --no-deps
	@echo -e "Removing non-lambda C-extension library files generated by pip install under target directory."
	@rm -rf ${target_dir}/nodeps/solarwinds_apm/extension/*.so*
