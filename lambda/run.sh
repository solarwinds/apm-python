...
main() {
	...
	stack='solarwinds-apm-python-stack'
	layerName='solarwinds-apm-python-layer'
	...
	if [[ $deploy == true ]]; then
		sam deploy --stack-name "$stack" --region "$region" --capabilities CAPABILITY_NAMED_IAM --parameter-overrides LayerName="$layerName"
		rm -f "$layerName".zip
	fi
