...
class TestW3CTransformer:
    def test_span_from_int(self):
        assert (
            W3CTransformer.span_id_from_int(1111222233334444)
            == f"{1111222233334444:016x}"
        )
    ...
    def test_span_id_from_sw_invalid_type_returns_zero_fallback(self):
        assert W3CTransformer.span_id_from_sw(None) == f"{0:016x}"
    ...
    def test_traceparent_from_context(self, span_context):
        assert (
            ...
            == f"00-{span_context.trace_id:032x}-{span_context.span_id:016x}-{span_context.trace_flags:02x}"
        )

    def test_sw_from_context(self, span_context):
        assert (
            W3CTransformer.sw_from_context(span_context)
            == f"{span_context.span_id:016x}-{span_context.trace_flags:02x}"
        )

    def test_sw_from_span_and_decision(self):
        assert W3CTransformer.sw_from_span_and_decision(
            1234, "01"
        ) == "{:016x}-{}".format(1234, "01")

    def test_remove_response_from_sw_key_present(self):
        ts = TraceState([["bar", "456"], ["xtrace_options_response", "123"]])
        assert W3CTransformer.remove_response_from_sw(ts) == TraceState(
            [["bar", "456"]]
        )
