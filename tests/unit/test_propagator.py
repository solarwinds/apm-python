...
class TestSolarWindsPropagator:
    ...
    def test_extract_existing_context(self):
        ...
        result = SolarWindsPropagator().extract(
            mock_carrier, mock_otel_context
        )
    ...
    def mock_otel_context(
        self, mocker, valid_span_id=True, trace_flags=0x01, trace_state=None
    ):
        ...
        mock_get_span_context.is_valid = (
            bool(valid_span_id) and mock_get_span_context.trace_id != 0
        )
    ...
    def test_inject_existing_tracestate_no_sw(self, mocker):
        ...
        trace_state = TraceState(
            [("xtrace_options_response", "abc123"), ("foo", "bar")]
        )
    ...
    def test_inject_existing_tracestate_existing_sw(self, mocker):
        ...
        trace_state = TraceState(
            [
                ("xtrace_options_response", "abc123"),
                ("foo", "bar"),
                ("sw", "some-existing-value"),
            ]
        )
        self.mock_otel_context(mocker, True, trace_state=trace_state)
        mock_carrier = {}
...
class TestSolarWindsPropagatorNonDictCarriers:
    ...
    def test_inject_list_carrier_no_existing_tracestate(self, mocker):
        ...
        tracestate_entries = [
            item for item in mock_carrier if item[0] == "tracestate"
        ]
    ...
    def test_inject_list_carrier_updates_existing_sw(self, mocker):
        ...
        existing_trace_state = TraceState(
            [("sw", "old-value"), ("foo", "bar")]
        )
        self.mock_otel_context_with_tracestate(mocker, existing_trace_state)
        mock_carrier = []
        mock_context = mocker.Mock()
        ...
        try:
            ...
            if "'list' object has no attribute 'get'" in str(e):
                raise AssertionError(
                    "Propagator incorrectly called carrier.get() on list carrier"
                ) from e
            raise
    ...
    def test_inject_custom_carrier_type(self, mocker):
        ...
        assert (
            mock_carrier.headers["tracestate"]
            == "sw=1000100010001000-01,existing=value"
        )
    ...
    def test_inject_list_carrier_no_duplicate_tracestate(self, mocker):
        ...
        with tracer.start_as_current_span(
            ...
            tracestate_writes = [
                call
                for call in tracking_setter.calls
                if call[0] == "tracestate"
            ]
            assert len(tracestate_writes) == 1, (
                f"Expected 1 tracestate write, got {len(tracestate_writes)}: {tracestate_writes}"
            )
            tracestate_entries = [
                item for item in carrier if item[0] == "tracestate"
            ]
            assert len(tracestate_entries) == 1, (
