...
class TestResponseTimeProcessor:
    ...
    def test_is_span_http_true(self, mocker):
        ...
        mock_span.configure_mock(
            **{"kind": "foo", "attributes": {"http.method": "bar"}}
        )
    ...
    def test_is_span_http_false_not_server_kind(self, mocker):
        ...
        mock_span = mocker.Mock()
        mock_span.configure_mock(
            **{"kind": "not-foo-hehe", "attributes": {"http.method": "bar"}}
        )
        processor = ResponseTimeProcessor(mocker.Mock())
    ...
    def test_is_span_http_false_no_server_kind_no_method(self, mocker):
        ...
        mock_span.configure_mock(
            **{
                "kind": "not-foo-hehe",
                "attributes": {"NOT.http.method.hehehehe": "bar"},
            }
        )
        processor = ResponseTimeProcessor(mocker.Mock())
        assert False == processor.is_span_http(mock_span)
    ...
    def test_get_http_status_code_from_span(self, mocker):
        ...
        mock_spanattributes.configure_mock(**{"HTTP_STATUS_CODE": "http.status_code"})
        mock_span = mocker.Mock()
        mock_span.configure_mock(
            **{"kind": "foo", "attributes": {"http.status_code": "foo"}}
        )
        processor = ResponseTimeProcessor(mocker.Mock())
    ...
    def test_get_http_status_code_default(self, mocker):
        ...
        mock_span = mocker.Mock()
        mock_span.configure_mock(
            **{"kind": "foo", "attributes": {"NOT.http.status_code.muahaha": "foo"}}
        )
        processor = ResponseTimeProcessor(mocker.Mock())
    ...
    def test_calculate_otlp_transaction_name_env_trans_truncated(self, mocker):
        ...
        assert (
            "foo-txn-ffoooofofooooooofooofooooofofofofoooooofoooooooooffoffooooooffffofooooofffooooooofoooooffoofofoooooofffofooofoffoooofooofoooooooooooooofooffoooofofooofoooofoofooffooooofoofooooofoooooffoofffoffoooooofoooofoooffooffooofofooooooffffooofoooooofoooooo"
    ...
    def test_calculate_otlp_transaction_name_env_lambda(self, mocker):
        ...
        assert (
            "foo-lambda-ffoooofofooooooofooofooooofofofofoooooofoooooooooffoffooooooffffofooooofffooooooofoooooffoofofoooooofffofooofoffoooofooofoooooooooooooofooffoooofofooofoooofoofooffooooofoofooooofoooooffoofffoffoooooofoooofoooffooffooofofooooooffffooofoooooofooo"
            == ResponseTimeProcessor(
                mock_apm_config,
            ).calculate_otlp_transaction_name("foo-span")
        )
    ...
    def patch_for_on_end(
        ...
        if missing_http_attrs:
            mock_basic_span.configure_mock(
                **{
                    "attributes": {"http.method": None, "TransactionName": "foo"},
                }
            )
        else:
            mock_basic_span.configure_mock(
        ...
        return mock_txname_manager, mock_apm_config, mock_histogram, mock_basic_span

    def test_on_end_valid_local_parent_span(self, mocker):
        """Only scenario to skip OTLP metrics generation (not entry span)"""
        mock_txname_manager, mock_apm_config, mock_histogram, _ = self.patch_for_on_end(
            mocker,
        )
        mock_span = mocker.Mock()
    ...
    def test_on_end_valid_remote_parent_span(self, mocker):
        ...
        mock_span.configure_mock(
            **{
                "parent": mock_parent,
                "attributes": {"http.method": "foo-method", "TransactionName": "foo"},
            }
        )
    ...
    def test_on_end_missing_txn_name(self, mocker):
        mock_txname_manager, mock_apm_config, mock_histogram, mock_basic_span = (
