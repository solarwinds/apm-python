...
def make_request_headers(options=MakeRequestHeaders()) -> RequestHeaders:
    if (
        options.trigger_trace is None
        and options.kvs is None
        and options.signature is None
    ):
    ...
    if (
        isinstance(options.signature_key, str)
        and options.signature == "bad-timestamp"
    ):
        timestamp -= 10 * 60
    ts = f"ts={timestamp}"
    ...
    if options.signature:
        key = (
            ...
            if options.signature_key
            else os.urandom(8).hex()
        )
        headers.x_trace_options_signature = hmac.new(
            str.encode(key), str.encode(headers.x_trace_options), hashlib.sha1
        ).hexdigest()
    return headers
...
def check_counters(sampler, counter_names):
    ...
    if counters == set():
        ...
        sampling_scope_metrics = [
            ...
            if sm.scope.name == "sw.apm.sampling.metrics"
        ]
...
class MockSampler(OboeSampler):
    ...
    def _create_parent(
        ...
        is_remote=False,
        sw=None,
        other_trace_state=False,
        xtrace_options_response=False,
    ) -> Context | None:
    ...
    def _create_parent_span(
            ...
            trace_state = TraceState(
                [
                    (
                        ...
                        format(span_id, "016x")
                        + "-0"
                        + ("0" if trace_flags == TraceFlags.SAMPLED else "1"),
                    )
                ]
            )
            ...
            trace_state = TraceState(
                [
                    (
                        ...
                        format(span_id, "016x")
                        + "-0"
                        + ("1" if trace_flags == TraceFlags.SAMPLED else "0"),
                    )
                ]
            )
        if other_trace_state:
            if trace_state is None:
                trace_state = TraceState()
            trace_state = trace_state.add("vendor1", "value1").add(
                "vendor2", "value2"
            )
        if xtrace_options_response:
            ...
            trace_state = trace_state.add(
                INTL_SWO_X_OPTIONS_RESPONSE_KEY, "response"
            )
...
class TestInvalidXTraceOptionsSignature:
    def test_rejects_missing_signature_key(self):
        sampler = MockSampler(
            MockSamplerOptions(
                ...
                local_settings=LocalSettings(
                    trigger_mode=True, tracing_mode=None
                ),
        ...
        ctxt = sampler._create_parent(
            trace_flags=TraceFlags.SAMPLED, is_remote=True
        )
        ...
        assert (
            ...
            in sampler.response_headers.x_trace_options_response
        )
...
class TestMissingSettings:
    def test_does_not_sample(self):
        sampler = MockSampler(
            MockSamplerOptions(
                settings=None,
                local_settings=LocalSettings(
                    trigger_mode=False, tracing_mode=None
                ),
        ...
        generator = RandomIdGenerator()
        sample = sampler.should_sample(
            None, generator.generate_trace_id(), "does_not_sample"
        )
        assert not sample.decision.is_sampled()
        assert not sample.decision.is_recording()
        check_counters(sampler, ["trace.service.request_count"])
    ...
    def test_expires_after_ttl(self):
        ...
        ctxt = sampler._create_parent(
            trace_flags=TraceFlags.SAMPLED, is_remote=True, sw=True
        )
        sample = sampler.should_sample(
            ctxt,
            get_current_span(ctxt).get_span_context().trace_id,
    ...
    def test_ignores_trigger_trace(self):
        ...
        generator = RandomIdGenerator()
        sample = sampler.should_sample(
            None, generator.generate_trace_id(), "ignores_trigger_trace"
        )
...
class TestEntrySpan:
    def test_sw_w3c_tracestate_with_x_trace_options_response(self):
        ...
        ctxt = sampler._create_parent(
            ...
            is_remote=True,
            sw=True,
            other_trace_state=True,
            xtrace_options_response=True,
        )
        sample = sampler.should_sample(
            ctxt,
            get_current_span(ctxt).get_span_context().trace_id,
        ...
        assert "vendor2=value2,vendor1=value1" in sample.attributes.get(
            TRACESTATE_CAPTURE_ATTRIBUTE
        )
        assert "sw=" in sample.attributes.get(TRACESTATE_CAPTURE_ATTRIBUTE)
        assert INTL_SWO_X_OPTIONS_RESPONSE_KEY not in sample.attributes.get(
            TRACESTATE_CAPTURE_ATTRIBUTE
        )
...
class TestEntrySpanWithValidSwContextSampleThroughAlwaysSet:
    ...
    def test_respects_parent_sampled(self, sample_through_always_set):
        ctxt = sample_through_always_set._create_parent(
            trace_flags=TraceFlags.SAMPLED, is_remote=True, sw=True
        )
    ...
    def test_respects_parent_not_sampled(self, sample_through_always_set):
        ctxt = sample_through_always_set._create_parent(
            trace_flags=TraceFlags.DEFAULT, is_remote=True, sw=True
        )
        sample = sample_through_always_set.should_sample(
            ctxt,
            get_current_span(ctxt).get_span_context().trace_id,
        ...
        check_counters(
            sample_through_always_set, ["trace.service.request_count"]
        )

    def test_respects_sw_sampled_over_w3c_not_sampled(
        self, sample_through_always_set
    ):
        ctxt = sample_through_always_set._create_parent(
            trace_flags=TraceFlags.DEFAULT, is_remote=True, sw="inverse"
        )
        sample = sample_through_always_set.should_sample(
            ctxt,
            get_current_span(ctxt).get_span_context().trace_id,
    ...
    def test_respects_sw_not_sampled_over_w3c_sampled(
        self, sample_through_always_set
    ):
        ctxt = sample_through_always_set._create_parent(
            trace_flags=TraceFlags.SAMPLED, is_remote=True, sw="inverse"
        )
        sample = sample_through_always_set.should_sample(
            ctxt,
            get_current_span(ctxt).get_span_context().trace_id,
        ...
        sampler = MockSampler(
            MockSamplerOptions(
                settings=Settings(
                    ...
                    buckets={
                        BucketType.TRIGGER_STRICT: BucketSettings(
                            capacity=10, rate=5
                        ),
                        BucketType.TRIGGER_RELAXED: BucketSettings(
                            capacity=0, rate=0
                        ),
                    },
                    signature_key=None,
...
class TestTriggerTraceRequestedDiceRoll:
    def test_respects_x_trace_options_keys_and_values(self):
        ...
        ctxt = sampler._create_parent(
            trace_flags=TraceFlags.DEFAULT, is_remote=True
        )
        sample = sampler.should_sample(
            ctxt,
            get_current_span(ctxt).get_span_context().trace_id,
    ...
    def test_records_and_samples_when_dice_success_and_sufficient_capacity(
        self,
    ):
        sampler = MockSampler(
            MockSamplerOptions(
                settings=Settings(
                    ...
                    flags=Flags.SAMPLE_START,
                    buckets={
                        BucketType.DEFAULT: BucketSettings(
                            capacity=10, rate=5
                        ),
                    },
                    signature_key=None,
    ...
    def test_records_but_does_not_sample_when_dice_success_but_insufficient_capacity(
        self,
    ):
        sampler = MockSampler(
            MockSamplerOptions(
    ...
    def test_records_but_does_not_sample_when_dice_failure(self):
        ...
        check_counters(
            sampler,
            ["trace.service.request_count", "trace.service.samplecount"],
        )
...
class TestTriggerTraceRequestedSampleStartUnset:
    def test_ignores_trigger_trace(self):
        ...
        assert sample.attributes.get("custom-key") == "value"
        assert (
            "trigger-trace=tracing-disabled"
            in sampler.response_headers.x_trace_options_response
        )
        assert (
            "ignored=invalid-key"
