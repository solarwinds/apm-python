...
def check_counters(sampler, counter_names):
    ...
    if counters == set():
        ...
        assert (
            len(
                sampler.metric_reader.get_metrics_data()
                .resource_metrics[0]
                .scope_metrics
            )
            == 1
    ...
    def _create_parent(
        ...
        return trace.set_span_in_context(
            self._create_parent_span(
                trace_flags, is_remote, sw, other_trace_state, xtrace_options_response
            )
        )
...
class TestInvalidXTraceOptionsSignature:
    def test_rejects_missing_signature_key(self):
        sampler = TestSampler(
            TestSamplerOptions(
                ...
                request_headers=make_request_headers(
                    MakeRequestHeaders(
                        trigger_trace=True, signature=True, kvs={"custom-key": "value"}
                    )
                ),
            )
        )
        ...
        assert (
            "auth=no-signature-key" in sampler.response_headers.x_trace_options_response
        )
...
class TestMissingSettings:
    ...
    def test_ignores_trigger_trace(self):
        ...
        assert (
            "ignored=invalid-key" in sampler.response_headers.x_trace_options_response
        )
...
class TestEntrySpanWithValidSwContextXTraceOptions:
    ...
    def test_ignores_trigger_trace(self):
        ...
        assert sample.attributes.get("custom-key") == "value"
        assert (
            "trigger-trace=ignored" in sampler.response_headers.x_trace_options_response
        )
        assert (
            "ignored=invalid-key" in sampler.response_headers.x_trace_options_response
        )
...
class TestTriggerTraceRequestedDiceRoll:
    ...
    def test_records_but_does_not_sample_when_dice_failure(self):
        sampler = TestSampler(
            TestSamplerOptions(
                settings=Settings(
                    ...
                    sample_source=SampleSource.LOCAL_DEFAULT,
                    flags=Flags.SAMPLE_START,
                    buckets={BucketType.DEFAULT: BucketSettings(capacity=10, rate=5)},
                    signature_key=None,
                    timestamp=int(time.time()),
                    ttl=10,
                ),
        ...
        check_counters(
            sampler, ["trace.service.request_count", "trace.service.samplecount"]
        )
