...
def mock_sys(mocker):
    ...
    mock_sys = mocker.patch("solarwinds_apm.configurator.sys")
...
def mock_tracerprovider(mocker):
    return mocker.patch("solarwinds_apm.configurator.SolarwindsTracerProvider")
...
def mock_apmconfig_enabled_export_logs_false(mocker):
    return mocker.patch(
        "solarwinds_apm.configurator.SolarWindsApmConfig",
        get_apmconfig_mocks(mocker, export_logs_enabled=False),
    )
...
def mock_apmconfig_logs_enabled_none(mocker):
    return mocker.patch(
        "solarwinds_apm.configurator.SolarWindsApmConfig",
        get_apmconfig_mocks(mocker, export_logs_enabled=None),
    )
...
def mock_init_logging(mocker):
    return mocker.patch("solarwinds_apm.configurator._init_logging")
