...
function get_wheel(){
    ...
    then
        ...
        if [ -z "$PIP_INSTALL" ]; then
            ...
            echo -e "Only testing the cp38 aarch64 wheel under ${APM_ROOT}"
            tested_wheel=$(find "$APM_ROOT"/dist/* -name "solarwinds_apm-$SOLARWINDS_APM_VERSION-cp38-cp38-manylinux_2_17_aarch64.manylinux2014_aarch64.whl")
        else
